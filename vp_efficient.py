...
class VPTree:
    ...
    def get_n_nearest_neighbors(self, query, n_neighbors):
        ...
        neighbors = []
        nodes_to_visit = deque([(self, 0)])

        furthest_d = np.inf

        while len(nodes_to_visit) > 0:
            node, d0 = nodes_to_visit.popleft()
            if node is None or d0 > furthest_d:
                continue

            d = self.dist_fn(query, node.vp)
            if d < furthest_d:
                heapq.heappush(neighbors,(d,node.vp))
            ...
            if node.left_min <= d <= node.left_max:
                nodes_to_visit.appendleft((node.left,0))
            ...
            if node.right_min <= d <= node.right_max:
            	nodes_to_visit.appendleft((node.right,0))
        ...
        for i in range(n_neighbors):
            answer.append(heapq.heappop(neighbors)) 
        return answer
